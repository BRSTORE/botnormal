...
module.exports.run = async (client, message, args) => {
  ...
  if (!args[0]) {
    const embedB = new Discord.MessageEmbed()
      .setColor("#4cd8b2")
      .setDescription("**Passo 1:** escolha uma das cores disponíveis no chat de <#602006904801329172>.\n**Passo 2:** use o comando `$$color` e escreva o nome da cor desejada na frente.")
      .setFooter("Exemplo: $$color preto");
    await message.channel.send(embedB);
  } else if (args[0].toLowerCase() === 'remove') {
  	await message.member.roles.remove(colors, "Comando $$color remove");
	    const embedRemove = new Discord.MessageEmbed()
        .setColor("#4cd8b2")
        .setDescription("**`" + message.author.username + "`,** suas cores foram removidas com sucesso!");
        return await message.channel.send(embedRemove);
  } else if (price === undefined || price === null) {
    const embedNop = new Discord.MessageEmbed()
      .setColor("#4cd8b2")
      .setDescription("**`" + str + "` não é uma poção de cor possível de se comprar.**");
    return message.channel.send(embedNop);
  } else /*if (buyer.coins < price) {
    const embedBuyer = new Discord.MessageEmbed()
      .setDescription(
        "**" +
      ...
      .setColor(role.color);
    return message.channel.send(embedBuyer);
  } else*/ if (!role) {
    ...
    message.channel
      ...
      .then(async collected => {
        if (collected.first().content.toLowerCase() === "sim") {
          //buyer.coins -= price;
          //buyer.save();
          await message.member.roles.remove(colors, "Trocando as cores com $$color");
          ...
          return message.channel.send(
            "**A compra foi rejeitada pelo usuário.**"
          );
        }
      })
      .catch(() => {
